...
class Trader:
    ...
    def get_data(
        self, symbol="BTCUSDT", interval=Client.KLINE_INTERVAL_1DAY, lookback=100
    ):
        klines = self.client.get_historical_klines(
            symbol, interval, f"{lookback} day ago UTC"
        )
        data = pd.DataFrame(
            klines,
            columns=[
    ...
    def trade(self, short_window, long_window):
        ...
        for i in range(1, len(self.df)):
            if (
                self.df[self.short_sma_column].iloc[i]
                > self.df[self.long_sma_column].iloc[i]
                and self.df[self.short_sma_column].iloc[i - 1]
                < self.df[self.long_sma_column].iloc[i - 1]
            ):
                # Buy signal
...
class Long_Short_Backtester:
    ...
    def __repr__(self):
        return "Long_Short_Backtester(symbol = {}, start = {}, end = {})".format(
    ...
    def optimize_strategy(
        self, SMA_S_range, SMA_M_range, SMA_L_range, metric="Multiple"
    ):
        self.metric = metric
        ...
        self.results_overview = pd.DataFrame(
            data=np.array(combinations), columns=["SMA_S", "SMA_M", "SMA_L"]
        )
    ...
    def find_best_strategy(self):
        ...
        print(
            "SMA_S: {} | SMA_M: {} | SMA_L : {} | {}: {}".format(
                SMA_S, SMA_M, SMA_L, self.metric, round(perf, 5)
            )
        )
    ...
    def print_performance(self):
        ...
        print(100 * "=")
        print(
            "TRIPLE SMA STRATEGY | INSTRUMENT = {} | SMAs = {}".format(
...
tester = Long_Short_Backtester(
    filepath="bitcoin.csv", symbol=symbol, start=start, end=end, tc=tc
)
...
class LongShortTrader:
    ...
    def stream_candles(self, msg):
        ...
        if (
            self.position != 0
            and len(self.prepared_data) > 0
            and len(self.prepared_data["position"]) > 0
        ):
            self.twm.stop()
            if self.position == 1:
